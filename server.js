...
socketEvent.on('connection', socket => {
    ...
    socket.on('join', user => {
        ...
        socket.broadcast.to(user.room).emit('newChat', { username: "System", message: user.username + " has joined" })
    })



    socket.on('leave', user => {
        socketEvent.to(user.room).emit('newChat', { username: "System", message: user.username + " has left" });
